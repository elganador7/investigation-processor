#!/usr/bin/env python3
"""
Test script to process a single investigation using Gemini for testing purposes.
This allows you to verify the API connection and output format before running the full analysis.
"""

import json
import os
from investigation_processor_gemini import InvestigationProcessorGemini
...
def test_single_investigation():
    """Test the Gemini processor with a single investigation."""
    ...
    try:
        ...
        if result['success']:
            ...
            # Save test result
            with open('test_result_gemini.json', 'w') as f:
                json.dump(result, f, indent=2)
            print(f"\n✅ Test result saved to test_result_gemini.json")
