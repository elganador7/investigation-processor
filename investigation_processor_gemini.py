...
class InvestigationProcessorGemini:
    def __init__(self):
    ...
    def call_gemini_api(self, prompt: str, investigation_title: str) -> Dict[str, Any]:
    ...
    def process_investigation(self, investigation: Dict[str, str], index: int) -> Dict[str, Any]:
        ...
        # Create the four separate prompts using shared prompts
        prompts = get_all_prompts(investigation)
        ...
        for section_name, prompt in prompts.items():
            ...
            try:
                section_result = self.call_gemini_api(prompt, f"{investigation['title']} - {section_name}")
    ...
    def process_all_investigations(self) -> List[Dict[str, Any]]:
        ...
        print(f"Starting analysis of {len(self.investigations)} investigations using Gemini...")
        ...
        for i, investigation in enumerate(self.investigations):
            ...
            self.save_results(results, f"investigation_results_gemini_progress_{i+1}.json")
            
            # Add delay between API calls to avoid rate limiting
            if i < len(self.investigations) - 1:
                print("Waiting 5 seconds before next investigation...")
                time.sleep(5)
    ...
    def save_results(self, results: List[Dict[str, Any]], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"investigation_results_gemini_{timestamp}.json"
    ...
    def generate_summary_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        report = f"""
# Investigation Analysis Summary Report (Gemini)
...
"""
    ...
    def save_summary_report(self, results: List[Dict[str, Any]]):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"investigation_summary_report_gemini_{timestamp}.md"
...
def main():
    ...
    try:
        ...
        processor = InvestigationProcessorGemini()
